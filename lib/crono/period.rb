...
module Crono
  ...
  class Period
    ...
    def next(since: nil) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength, Metrics/PerceivedComplexity
      if @interval
        if since
    ...
    def description
      ...
      desc += format(' at %.2i:%.2i', @at_hour, @at_min) if @at_hour && @at_min # rubocop:disable Style/FormatStringToken
    ...
    private
      ...
      def parse_on(on)
        ...
        raise "Wrong 'on' day" unless day_number
        raise "period should be at least 1 week to use 'on'" if @period < 1.week

        day_number
      end

      def parse_at(at) # rubocop:disable Metrics/MethodLength
        if @period < 1.day && (at.is_a? String || at[:hour]) # rubocop:disable Lint/RequireParentheses
          raise "period should be at least 1 day to use 'at' with specified hour"
        end
